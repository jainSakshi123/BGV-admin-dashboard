...
function BusinessIndustry() {
  ...
  const GetUsers = async () => {
    try {
      const data = await CompanyUserApi(
        `${mainUrl}/admin/bus-industry-list`,
        "GET"
      );

      if (data.status === true) {
        setShowSkeleton(false);
        setBusinessIndustry(data.businessIndustry);
  ...
  useEffect(() => {
    ...
    reload();
    GetUsers();
  }, [page, searchData, BusinessIndustry.length]);
  ...
  return (
    <>
      ...
      ) : BusinessIndustry?.options?.length > 0 ? (
        BusinessIndustry?.options?.map((option, index) => (
          <div
            key={index}
            className={`px-8 ${
              index !== BusinessIndustry.options?.length - 1 && "border-b"
            }`}
          >
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        ...
                        onClick={() =>
                          handleDelete(BusinessIndustry._id, option)
                        }
                      >
                        Continue
                      </AlertDialogAction>
