...
function ShowAnnualVolume() {
  ...
  const GetUsers = async () => {
    try {
      ...
      if (data.status === true) {
        ...
        setAnnualVolume(data.annualVolume);
  ...
  useEffect(() => {
    const reload = () => {
      if (annualvolume.length + 1) {
        GetUsers();
      }
    };
    reload();
    GetUsers();
  }, [page, searchData, annualvolume.length]);

  const handleDelete = async (annualVolumeId, option) => {
    try {
      const data = await CompanyUserApi(
        `${mainUrl}/admin/delete-annual-volume/${annualVolumeId}/options/${option}`,
        "DELETE"
      );
      ...
      if (data.status === true) {
        ...
        if (Array.isArray(annualVolumeId)) {
          setAnnualVolume((prevAnnualVolume) =>
            prevAnnualVolume.filter((volume) => volume._id !== annualVolumeId)
          );
        } else {
          console.error("annualVolume is not an array:", annualVolumeId);
        }
      }
    } catch (error) {
    ...
    <>
      <div className="px-8 text-end">
        <AddVolume
          ...
          postData={`${mainUrl}/admin/add-annual-volume`}
        />
      </div>
      ...
      ) : annualvolume?.options?.length > 0 ? (
        annualvolume?.options?.map((option, index) => (
          <div
            key={index}
            className={`px-8 ${
              index !== annualvolume?.options?.length - 1 && "border-b"
            }`}
          >
            <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center gap-2">
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        ...
                        onClick={() => handleDelete(annualvolume._id, option)}
                      >
                        Continue
                      </AlertDialogAction>
                    </AlertDialogFooter>
                ...
                <EditDrpdown
                  apiURL={`admin/edit-annual-volume`}
                  params={annualvolume._id}
                  option={option}
                  setContractorOrg={setAnnualVolume}
                />
              </div>
            </div>
          </div>
...
export default ShowAnnualVolume;
