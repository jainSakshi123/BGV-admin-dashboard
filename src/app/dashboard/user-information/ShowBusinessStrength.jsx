...
function ShowBusinessStrength() {
  ...
  const getBusinessOrg = async () => {
    try {
      const data = await CompanyUserApi(
        `${mainUrl}/admin/bus-org-strength-list`,
        "GET"
      );
      if (data.status === true) {
        setShowSkeleton(false);
        setBusinessOrg(data.businessOrgStrength);
  ...
  useEffect(() => {
    getBusinessOrg();
    const reload = () => {
      if (businessOrg.length + 1) {
        getBusinessOrg();
      }
    };

    reload();
  }, [page, searchData, businessOrg.length]);

  const handleDelete = async (businessVolumeId, option) => {
    try {
      const data = await CompanyUserApi(
        `${mainUrl}/admin/delete-bus-org-strength/${businessVolumeId}/options/${option}`,
        "DELETE"
      );
      ...
      if (data.status === true) {
        ...
        // Ensure businessVolume is an array before filtering
        if (Array.isArray(businessOrg)) {
          setBusinessOrg((prevbusinessVolume) =>
            prevbusinessVolume.filter(
              (volume) => volume._id !== businessVolumeId
            )
          );
        } else {
          console.error("businessVolume is not an array:", businessOrg);
        }
      }
    } catch (error) {
  ...
  return (
    <>
      <div className="px-8 text-end">
        <AddVolume
          setAnnualVolume={setBusinessOrg}
          postData={`${mainUrl}/admin/add-bus-org-strength`}
        />
      </div>
      {showSkeleton ? (
      ...
      ) : businessOrg?.options?.length > 0 ? (
        businessOrg?.options?.map((option, index) => (
          <div
            key={index}
            className={`px-8 ${
              index !== businessOrg.options?.length - 1 && "border-b"
            }`}
          >
            <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center gap-2">
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        ...
                        onClick={() => handleDelete(businessOrg._id, option)}
                      >
                        Continue
                      </AlertDialogAction>
                    </AlertDialogFooter>
                ...
                <EditDrpdown
                  apiURL={`admin/edit-bus-org-strength`}
                  params={businessOrg._id}
                  option={option}
                  setContractorOrg={setBusinessOrg}
                />
              </div>
            </div>
          </div>
...
export default ShowBusinessStrength;
